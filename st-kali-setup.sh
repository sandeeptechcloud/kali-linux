...
checkdeps() {
	...
	printf "\n${yellow} ╚═══════════════════════════════════════╝${reset}"
	printf "\n\n${blue} [*] Updating apt cache...${reset}\n"
	apt update -y &> /dev/null
	printf "${blue} [*] Checking for all required tools...${reset}\n"
	...
	printf "${green} [*] All dependencies checked!${reset}\n"
...
# ── Spinner: single line, short text, never wraps ─────────────
show_spinner() {
    local pid=$1
    local msg="$2"
    local spin='|/-\\'
    local i=0
    local elapsed=0
    # \r overwrites same line; keep text < 40 chars to avoid wrap
    while kill -0 "$pid" 2>/dev/null; do
        i=$(( (i+1) % 4 ))
        frame=$(printf '%s' "$spin" | cut -c$((i+1)))
        printf "\r${cyan}  %s  %-22s %4ds${reset}" "$frame" "$msg" "$elapsed"
        sleep 1
        elapsed=$((elapsed+1))
    done
    printf "\r${green}  ✓  %-22s %4ds${reset}\n" "$msg" "$elapsed"
}

# ── Silent stage runner: run cmd in background, show spinner ──
run_stage() {
    local label="$1"
    shift
    ( "$@" &>/dev/null 2>&1 ) &
...
extract() {
	...
	printf "\n${yellow}  ╔════════════════════════════════════════╗"
	printf "\n${yellow}  ║  📦 Extracting Kali Rootfs         ║"
	printf "\n${yellow}  ║  ⏱  Est. time: 10-15 mins          ║"
...
# ── Install cfonts silently with spinner animation ────────────
install_banner_deps() {
    local spin='|/-\\'
    local i=0
    local elapsed=0
    ...
    ( pkg install nodejs -y &>/dev/null 2>&1; npm install -g cfonts &>/dev/null 2>&1 ) &
    local BG_PID=$!

    # Animate while installing
    while kill -0 "$BG_PID" 2>/dev/null; do
        i=$(( (i+1) % 4 ))
        frame=$(echo "$spin" | cut -c$((i+1)))
        printf "\r${cyan}  %s  Installing... [%ds]${reset}  " "$frame" "$elapsed"
        sleep 1
        elapsed=$((elapsed+1))
    done
    wait "$BG_PID"
    printf "\r${green}  ✓  Dependencies ready! [%ds]${reset}       \n" "$elapsed"
    sleep 0
}

install_banner_deps

# ── Clear screen then show banner ────────────────────────────
clear
CFONTS_BIN="$PREFIX/bin/cfonts"
if [ -f "$CFONTS_BIN" ]; then
...
# Stage 1: Cleanup + arch detect (fast, no spinner needed)
pre_cleanup
checksysinfo
printf "${green}  ✓  Architecture: ${SETARCH}${reset}\n"

# Stage 2: Dependencies (checkdeps handles its own output)
checkdeps
setchroot
DESTINATION=$HOME/chroot/kali-$SETARCH

# Stage 3: Download rootfs
printf "\n${blue}  ──────────────────────────────────────${reset}\n"
printf "${cyan}  Downloading Kali rootfs...${reset}\n"
gettarfile
checkintegrity

# Stage 4: Extract (extract() handles its own spinner)
extract

# Stage 5: Create launch scripts (silent)
( createloginfile ) &
show_spinner "$!" "Creating scripts..."
wait

# Stage 6: Configure (silent)
printf "nameserver 8.8.8.8\nnameserver 8.8.4.4\n" > $DESTINATION/etc/resolv.conf
( finalwork ) &
show_spinner "$!" "Configuring Kali..."
wait
...
printline
printf "\n${yellow} ╔═══════════════════════════════════════════════════════╗\n"
printf "${yellow} ║   ✅ Installation Complete! Enjoy Kali  ║\n"
printf "${yellow} ╚═══════════════════════════════════════════════════════╝\n"
printf "\n"
printf "  ┌─────────────────────────────────────────────┐\n"
printf "  │  🎮 ${green}kali-linux${yellow}       → User Mode    │\n"
printf "  │  🎮 ${red}kali-linux-root${yellow}   → Root Mode    │\n"
printf "  └─────────────────────────────────────────────┘\n"
printf "\n"
printline
printf "${reset}\n"
