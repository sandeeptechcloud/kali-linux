...
# Install cfonts dependencies silently before showing banner
printf "${blue} [*] Setting up banner...${reset}\n"
apt-get install -y nodejs &>/dev/null || true
npm install -g cfonts &>/dev/null || true

echo ""
# Show cfonts banner if available, else fallback
if command -v cfonts &>/dev/null; then
    cfonts "SANDEEP" -f slick -c white,blue -a center
else
    echo -e "${cyan}  ======================================"
    echo -e "       S A N D E E P   T E C H"
    echo -e "  ======================================${reset}"
fi
echo ""
echo -e "${reset}  ╔══════════════════════════════════════════╗"
