...
checkdeps() {
	...
	loading_animation() {
		...
		while kill -0 $pid 2>/dev/null; do
			for i in $(seq 0 9); do
				printf "\r  ${yellow}[${spinChars:$i:1}] ${msg}...${reset}"
				sleep $delay
			done
		done
		printf "\r  ${green}[✓] ${msg} Complete!${reset}\n"
	}

	printf "\n${yellow} [*] Installing Node.js and cfonts...${reset}\n"

	# Install wget
	printf "  ${blue}→ Installing wget...${reset}"
	(apt-get install wget -y &>/dev/null) &
	wpid=$!
	loading_animation "wget" $wpid
	wait $wpid 2>/dev/null

	# Install nodejs
	printf "  ${blue}→ Installing Node.js...${reset}"
	(apt-get install nodejs -y &>/dev/null) &
	npid=$!
	loading_animation "nodejs" $npid
	wait $npid 2>/dev/null

	# Install cfonts
	printf "  ${blue}→ Installing cfonts...${reset}"
	(npm install cfonts -g &>/dev/null) &
	cpid=$!
	loading_animation "cfonts" $cpid
...
extract() {
	...
	# First, extract to home directory to see the structure
	proot --link2symlink tar -xf "$rootfs" -C $HOME 2>/dev/null || :

	# Find the extracted directory (could be named various ways)
	EXTRACTED_DIR=""
	for dir in "$HOME"/kali-nethunter-rootfs-* "$HOME"/kali-*; do
		if [ -d "$dir" ]; then
			EXTRACTED_DIR="$dir"
			break
		fi
	...
	if [ -n "$EXTRACTED_DIR" ]; then
		# Move extracted content to correct destination
		mv "$EXTRACTED_DIR"/* "$DESTINATION/" 2>/dev/null || mv "$EXTRACTED_DIR" "$DESTINATION" 2>/dev/null || :
		rm -rf "$EXTRACTED_DIR" 2>/dev/null || :
		printf "${green} [*] Extraction complete!${reset}\n"
	else
		printf "${red} [!] Failed to find extracted files${reset}\n"
	fi
}
