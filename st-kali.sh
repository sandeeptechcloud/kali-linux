...
DPCEOF
    ...
    # 4. Debconf noninteractive mode
    mkdir -p "$DESTINATION/etc/debconf.conf.d" 2>/dev/null || true
    echo 'DEBCONF_FRONTEND=noninteractive' >> "$DESTINATION/etc/debconf.conf" 2>/dev/null || true
    ...
    # 6. Set environment defaults for every Kali session
    {
        echo 'DEBIAN_FRONTEND=noninteractive'
        echo 'DEBCONF_FRONTEND=noninteractive'
        echo 'DEBCONF_NONINTERACTIVE_SEEN=true'
        echo 'LC_ALL=C.UTF-8'
        echo 'LANG=C.UTF-8'
